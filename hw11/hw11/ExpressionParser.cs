    ...
    public class ExpressionParser : IParser
    {
        ...
        public Expression ParseExpression(string stringToParse)
        {
            ...
            foreach (var symbol in stringToParse)
            {
                ...
                if (!TryParseOperationOrQuit(strSymbol, out var operation) && strSymbol != ")" && strSymbol != "(" && !double.TryParse(strSymbol, out var num))
                {
                    throw new InvalidSymbol("Invalid symbol here");
                }
                
                if (operation == Operation.None && strSymbol != ")" && strSymbol != "(")
                {
                    numberBeforeParse += strSymbol;
                    prevSymbol = strSymbol;
                    continue;
                }
                ...
                {
                    if (TryParseOperationOrQuit(prevSymbol, out operation))
                        throw new InvalidSyntax("Операция не может следовать за операцией");
                ...
                else if (symbol == ')')
                {
                    ...
                    throw new InvalidSmthngElse("Invalid data, try again");
